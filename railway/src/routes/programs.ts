	...
	try {
		...
		const { data, error } = await supabase
			...
			.insert({
				...
				judging_type: judging_type === 'auto_assign' ? 'auto_assign' : 'parallel',
...
router.put('/:id', requireAuth, requireProgramAccess('id'), async (req: AuthenticatedRequest, res) => {
	...
	try {
		...
		const updates = {
			...
			judging_type: req.body?.judging_type === 'auto_assign' ? 'auto_assign' : req.body?.judging_type === 'parallel' ? 'parallel' : undefined,
