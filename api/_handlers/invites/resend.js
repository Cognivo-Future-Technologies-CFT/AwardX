...
export default async function handler(req, res) {
    ...
    try {
        if (inviteType === 'team') {
            ...
            const programTitle = inviteRow.programs?.title || programTitleFallback || 'your workspace';
            const roleName = inviteRow.roles?.name || 'Team member';
            const subject = `Team invite: ${programTitle}`;
            let deadlineText = '';
            const programDeadline = inviteRow.programs?.deadline;
            if (programDeadline) {
  ...
  <body style="margin:0;padding:0;background-color:#f8fafc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;">
    ...
    <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color:#f8fafc;">
      <tr>
        ...
        const programTitle = judgeRow.programs?.title || programTitleFallback || 'your workspace';
        const judgeName = judgeRow.name || 'Judge';
        const subject = `You're invited to judge: ${programTitle}`;
        let deadlineText = '';
        const programDeadline = judgeRow.programs?.deadline;
        if (programDeadline) {
            const d = new Date(programDeadline);
            deadlineText = d.toLocaleDateString('en-US', {
                month: 'long',
