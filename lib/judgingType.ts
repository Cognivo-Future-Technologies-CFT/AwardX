...
export const DEFAULT_JUDGING_TYPE: JudgingType = 'parallel';

export const JUDGING_TYPE_OPTIONS: Array<{ value: JudgingType; label: string; description: string }> = [
  {
    value: 'parallel',
...
export function normalizeJudgingType(value?: string | null): JudgingType {
  return value === 'auto_assign' ? 'auto_assign' : 'parallel';
}

export function isAutoAssignJudging(value?: string | null): boolean {
  return normalizeJudgingType(value) === 'auto_assign';
}

export function getCategorySelectorLabels(isAutoAssign: boolean) {
  return {
