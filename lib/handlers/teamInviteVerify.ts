...
export async function resolveTeamInvite(supabase: any, token: string) {
  ...
  if (inviteError || !invite) {
    return { error: 'Invalid or expired invite link.' as const, statusCode: 404 };
  }
  ...
  return { invite: invite as TeamInviteRecord };
}

async function organizationNameForInvite(supabase: any, organizationId: string) {
  const { data: org } = await supabase.from('organizations').select('name').eq('id', organizationId).maybeSingle();
  return org?.name || 'Organization';
}

function inviteContext(invite: TeamInviteRecord, organizationName: string) {
  return {
...
export async function verifyTeamInviteGet(
  supabase: any,
  token: string,
...
export async function verifyTeamInvitePost(
  supabase: any,
  token: string,
  action: string,
