...
export async function verifyJudgeInvite(supabase, input) {
    ...
    const organizationName = String(orgResult.data?.name || '');
    ...
    if (assignments.length > 0) {
        const categoryIds = Array.from(new Set(assignments
            .map((row) => row.submissions?.category_id)
            .filter(Boolean)));
        ...
        const submissionIds = assignments.map((row) => row.submission_id).filter(Boolean);
        ...
        assignments = assignments.map((row) => ({
            ...row,
            category_name: categoryMap.get(row.submissions?.category_id || '') ||
                'Uncategorized',
    ...
    return serviceJson(200, {
        ...
        assignments: assignments.map((row) => {
            ...
            const roundInfo = row.round_info;
            return {
