...
export type JudgeSubmitScoresInput = {
  token: string;
  submissionJudgeId: string;
  criteriaScores: Array<{ criterionId: string; score: number; comment?: string }>;
  overallComment?: string;
};

export async function submitJudgeScores(
  supabase: any,
  input: JudgeSubmitScoresInput,
  ...
  const programId = (assignment as { submissions?: { program_id?: string } }).submissions?.program_id;
  ...
  const criteriaById = new Map(
    (criteriaRows || []).map((c: { id: string; min_score?: number | null; max_score?: number | null }) => [c.id, c]),
  );
  for (const cs of criteriaScores) {
    const criterion = criteriaById.get(cs.criterionId) as
      | { id: string; min_score?: number | null; max_score?: number | null }
      | undefined;
    if (!criterion) {
