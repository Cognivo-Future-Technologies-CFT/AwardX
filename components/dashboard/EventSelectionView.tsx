...
const ExistingEventCard: React.FC<{
  ...
  {event.topics?.[0] || "Uncategorized"} • {event.entriesCount} Entries
</p>
...
<label className="block text-sm font-semibold text-slate-700 mb-3">
   ...
   <div className="flex flex-wrap gap-2">
      {TOPICS.map((topic) => {
         const selected = newEvent.topics?.includes(topic);
            ...
            <button
               ...
               onClick={() =>
                  setNewEvent({
                     ...newEvent,
                     topics: selected
                        ? newEvent.topics.filter((t) => t !== topic)
                        : [...(newEvent.topics || []), topic],
                  })
               }
   ...
   <p className="text-xs text-slate-500 mt-2">
      Select one or more topics that describe this event.
   </p>
</div>
               </div>
               <div>
